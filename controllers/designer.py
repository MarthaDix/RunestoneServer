...
import requests, json, time, re, importlib, os
...
def update_local_files(path,clone_url,commit_id,oldbookname = None):
    print(path,clone_url,commit_id)
    if oldbookname:
    ...
    try:
        ...
        reset_local(path,clone_url,commit_id)
    print("end of update local files")

def update_books_webhook():
    print("IN UPDATEBOOKSWEBHOOK")
    print(request.env.HTTP_X_HUB_SIGNATURE)
    #print(request.env.http_accept)
    #print(request.env)
    #print(type(request.vars.repository))
    #print(request.vars['repository']['default_branch'])
    post_data_json= request.vars
    print(post_data_json.sender)
    print(post_data_json['repository']['owner']['login'], post_data_json['repository']['name'])
    ...
    if textbook and ('ref' not in post_data_json.keys() or post_data_json['repository']['default_branch'] == post_data_json['ref'].split("/")[-1]):
        print("here")
        #print(post_data_json['repository']['name'],post_data_json['repository']['owner'])
        local_path = "applications/runestone/custom_books/drafts/"+textbook.path
        print("here2")
        print(post_data_json['repository']['clone_url'])
        #print(local_path,post_data_json['repository']['clone_url'],post_data_json['repository']['name'],post_data_json['repository']['commits']['head_commit']['id'])
        if 'commits' in post_data_json.keys():
            print("if")
            print(post_data_json['head_commit']['id'])
...
def github_book_repo(account, repo, oldb, renameval= None, delete=False, reset=False, create =False):
    ...
    if create or reset:
        ...
        hook_data = {"config":{"url":"https://runestone.academy/runestone/designer/update_books_webhook"}}
...
def book_edit():
    ...
    if request.vars.changeType == "create":
        ...
        if changed_github['failed']:
            ...
            print("b4")
            print()
            print(request.vars.githubUser)
            print(session.auth.user.username)
            print(request.vars.newBookIdentifier)
            print(request.vars.baseBook)
            db.textbooks.update_or_insert(
        ...
        if changed_github['failed']:
            ...
            #local_path = query_for_path(post_data_json['repository']['owner']['login'], post_data_json['repository']['name'])
            print(existing_old_book.draft_commit)
...
def gather_book_info(book_list,username=None):
    ...
    for book in sorted(book_list):
        ...
        if not username: # non-custom book
            ...
            print("gang")
            book_info["github_repo_name"] = (db(db.textbooks.path == session.auth.user.username +"/"+ book).select().first()).github_repo_name
            book_info["github_account"] = (db(db.textbooks.path == session.auth.user.username +"/"+ book).select().first()).github_account
